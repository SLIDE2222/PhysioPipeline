...
function renderAdminReviewCard(review, type) {
  ...
  const profileHref = review?.profile?.id
    ...
    : 'profile.html';
  const location = [review?.profile?.city, review?.profile?.neighborhood].filter(Boolean).join(' • ') || 'Localização não informada';
...
async function loadAdminReviews() {
  ...
  try {
    const [pendingResponse, reportedResponse] = await Promise.all([
      window.physioApi.fetchAdminReviews('pending'),
    ...
    adminPendingReviewsList.innerHTML = pendingReviews.length
      ? pendingReviews.map((review) => renderAdminReviewCard(review, 'pending')).join('')
    ...
    setAdminReviewsMessage(error?.message || 'Não foi possível carregar a fila de reviews.', 'error');
    adminPendingReviewsList.innerHTML = '<p class="form-hint">Não foi possível carregar reviews pendentes.</p>';
    adminReportedReviewsList.innerHTML = '<p class="form-hint">Não foi possível carregar reviews reportadas.</p>';
  }
}
...
async function ensureAdminAccess() {
  ...
  if (!user.isAdmin) {
    setAdminReviewsMessage('Acesso restrito à administração.', 'error');
    adminPendingReviewsList.innerHTML = '<p class="form-hint">Você não tem permissão para moderar reviews.</p>';
    adminReportedReviewsList.innerHTML = '<p class="form-hint">Você não tem permissão para moderar reviews.</p>';
    return false;
  }
...
document.addEventListener('click', async (event) => {
  ...
  try {
    ...
    setAdminReviewsMessage(error?.message || 'Não foi possível moderar esta review.', 'error');
  } finally {
